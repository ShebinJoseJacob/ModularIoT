...
void httpRequest() {
  ...
  float voltage = smoke * (5.0 / 1023.0);
  float cleanAirVoltage = 0 .0;
  float dirtyAirVoltage = 5.0;
  
  // Calculate air quality index based on voltage values
  int airQualityIndex = map(voltage, cleanAirVoltage, dirtyAirVoltage, 100, 0);
  ...
  if (client.connect(server, 5000)) {
    ...
    client.print("MQ2:"+String(airQualityIndex));
